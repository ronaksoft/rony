...
func (g *Generator) genSave(m *protogen.Message) {
	// SaveWithTxn func
	g.g.P("func Save", m.Desc.Name(), "WithTxn (txn *rony.StoreTxn, alloc *tools.Allocator, m *", m.Desc.Name(), ") (err error) {")
	g.g.P("if alloc == nil {")
	...
	g.g.P("alloc := tools.NewAllocator()")
	g.g.P("defer alloc.ReleaseAll()")
	g.g.P("return store.Update(func(txn *rony.StoreTxn) error {")
...
func (g *Generator) genRead(m *protogen.Message) {
	g.g.P("func Read", m.Desc.Name(), "WithTxn (txn *rony.StoreTxn, alloc *tools.Allocator, m *", m.Desc.Name(), ") (*", m.Desc.Name(), ",error) {")
	g.g.P("if alloc == nil {")
	g.g.P("alloc = tools.NewAllocator()")
	g.g.P("defer alloc.ReleaseAll()")
	g.g.P("}") // end of if block
	...
	g.g.P("}")
	g.g.P()
	g.g.P("err := store.View(func(txn *rony.StoreTxn) (err error) {")
