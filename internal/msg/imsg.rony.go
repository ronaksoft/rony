...
func (r *PageLocalRepo) ListWithTxn(
	txn *rony.StoreTxn, alloc *tools.Allocator, pk PagePrimaryKey, lo *store.ListOption, cond func(m *Page) bool,
) ([]*Page, error) {
	if alloc == nil {
		alloc = tools.NewAllocator()
		defer alloc.ReleaseAll()
	...
	case PageReplicaSetIDPK:
		...
		seekKey = alloc.Gen('M', C_Page, 1040696757, pk.ReplicaSet, pk.ID)
...
func (r *PageLocalRepo) IterWithTxn(
	txn *rony.StoreTxn, alloc *tools.Allocator, pk PagePrimaryKey, ito *store.IterOption, cb func(m *Page) bool,
) error {
	if alloc == nil {
		alloc = tools.NewAllocator()
		defer alloc.ReleaseAll()
