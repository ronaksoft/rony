...
func (r *PageLocalRepo) ListWithTxn(
	txn *rony.StoreTxn, alloc *tools.Allocator, pk PagePartKey, lo *store.ListOption, cond func(m *Page) bool,
) ([]*Page, error) {
	if alloc == nil {
		alloc = tools.NewAllocator()
		defer alloc.ReleaseAll()
	...
	case PagePK:
		...
		seekKey = alloc.Gen('M', C_Page, 299066170, pk.ID)

	case PageReplicaSetIDPK:
		opt.Prefix = alloc.Gen('M', C_Page, 1040696757)
		seekKey = alloc.Gen('M', C_Page, 1040696757, pk.ReplicaSet)
...
func (r *PageLocalRepo) List(
	pk PagePartKey, lo *store.ListOption, cond func(m *Page) bool,
) ([]*Page, error) {
	alloc := tools.NewAllocator()
	defer alloc.ReleaseAll()
...
func (r *PageLocalRepo) IterWithTxn(
	txn *rony.StoreTxn, alloc *tools.Allocator, pk PagePartKey, ito *store.IterOption, cb func(m *Page) bool,
) error {
	if alloc == nil {
		alloc = tools.NewAllocator()
		defer alloc.ReleaseAll()
...
func (r *PageLocalRepo) Iter(
	pk PagePartKey, ito *store.IterOption, cb func(m *Page) bool,
) error {
	alloc := tools.NewAllocator()
	defer alloc.ReleaseAll()
