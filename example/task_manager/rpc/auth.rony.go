...
func (sw *authWrapper) registerRestClient(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	req := PoolRegisterRequest.Get()
	defer PoolRegisterRequest.Put(req)

	ctx.FillEnvelope(conn.ConnID(), C_AuthRegister, req)
	return nil
}
func (sw *authWrapper) registerRestServer(conn rony.RestConn, ctx *edge.DispatchCtx) error {
...
func (sw *authWrapper) loginRestClient(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	req := PoolLoginRequest.Get()
	defer PoolLoginRequest.Put(req)

	ctx.FillEnvelope(conn.ConnID(), C_AuthLogin, req)
	return nil
}
func (sw *authWrapper) loginRestServer(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	envelope := ctx.BufferPop()
	if envelope == nil {
		return errors.ErrInternalServer
	}
