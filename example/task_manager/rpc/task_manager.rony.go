...
func init() {
	...
	registry.RegisterConstructor(35362179, "TaskManagerRegister")
	registry.RegisterConstructor(656640319, "TaskManagerLogin")
...
func (sw *taskManagerWrapper) registerWrapper(ctx *edge.RequestCtx, in *rony.MessageEnvelope) {
...
func (sw *taskManagerWrapper) loginWrapper(ctx *edge.RequestCtx, in *rony.MessageEnvelope) {
...
func (sw *taskManagerWrapper) Register(e *edge.Server, handlerFunc func(c int64) []edge.Handler) {
	...
	e.SetHandler(
		edge.NewHandlerOptions().SetConstructor(C_TaskManagerRegister).
			SetHandler(handlerFunc(C_TaskManagerRegister)...).
			Append(sw.registerWrapper),
	)
	e.SetHandler(
		edge.NewHandlerOptions().SetConstructor(C_TaskManagerLogin).
			SetHandler(handlerFunc(C_TaskManagerLogin)...).
...
func TunnelRequestTaskManagerRegister(ctx *edge.RequestCtx, replicaSet uint64, req *RegisterRequest, res *Authorization, kvs ...*rony.KeyValue) error {
	...
	out.Fill(ctx.ReqID(), C_TaskManagerRegister, req, kvs...)
...
func TunnelRequestTaskManagerLogin(ctx *edge.RequestCtx, replicaSet uint64, req *LoginRequest, res *Authorization, kvs ...*rony.KeyValue) error {
	...
	in := rony.PoolMessageEnvelope.Get()
	defer rony.PoolMessageEnvelope.Put(in)
	out.Fill(ctx.ReqID(), C_TaskManagerLogin, req, kvs...)
	err := ctx.TunnelRequest(replicaSet, out, in)
	if err != nil {
		return err
	}
...
func (sw *taskManagerWrapper) registerRestClient(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	req := PoolRegisterRequest.Get()
	defer PoolRegisterRequest.Put(req)

	ctx.FillEnvelope(conn.ConnID(), C_TaskManagerRegister, req)
	return nil
}
func (sw *taskManagerWrapper) registerRestServer(conn rony.RestConn, ctx *edge.DispatchCtx) error {
...
func (sw *taskManagerWrapper) loginRestClient(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	req := PoolLoginRequest.Get()
	defer PoolLoginRequest.Put(req)

	ctx.FillEnvelope(conn.ConnID(), C_TaskManagerLogin, req)
	return nil
}
func (sw *taskManagerWrapper) loginRestServer(conn rony.RestConn, ctx *edge.DispatchCtx) error {
	envelope := ctx.BufferPop()
	if envelope == nil {
		return errors.ErrInternalServer
	}
...
func (c *TaskManagerClient) Register(req *RegisterRequest, kvs ...*rony.KeyValue) (*Authorization, error) {
	...
	in := rony.PoolMessageEnvelope.Get()
	defer rony.PoolMessageEnvelope.Put(in)
	out.Fill(c.c.GetRequestID(), C_TaskManagerRegister, req, kvs...)
	err := c.c.Send(out, in)
	if err != nil {
		return nil, err
	}
...
func (c *TaskManagerClient) Login(req *LoginRequest, kvs ...*rony.KeyValue) (*Authorization, error) {
	...
	in := rony.PoolMessageEnvelope.Get()
	defer rony.PoolMessageEnvelope.Put(in)
	out.Fill(c.c.GetRequestID(), C_TaskManagerLogin, req, kvs...)
	err := c.c.Send(out, in)
	if err != nil {
		return nil, err
	}
